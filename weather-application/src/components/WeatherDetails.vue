

<template>
    <div class="root-div">

        <!-- DIV TO HOLD THE BACKGROUND IMAGE -->
        <div class="bg-image-container">
            <img :src="bgImage" alt="">
        </div>

        <div class="content-container">

            <!-- COMPONENT -->
            <CityNameComp @passCityName="fetchCityWeatherDetails" />

            <div class="weather-details-container">
                <div class="weather-details-content">

                    <div class="title-div">
                        <h2 v-show="currentCityName!==''">weather in {{currentCityName}}</h2>
                    </div>

                    <div class="temp-div">
                        <h1 v-show="temp!==''" >{{temp}}  <span class="temp-span"><sup>o</sup>C</span> </h1>
                    </div>
                    
                    <div class="description-div">
                        <img :src="weatherIcon" alt="">
                        <h1 v-show="weatherDescription!==''"> {{weatherDescription}}</h1>
                    </div>
                    
                    <div class="humidity-div">
                        <h1 v-show="humidity!==''">humidity: {{humidity}} <span class="humidity-span">%</span> </h1>
                    </div>
                    
                    <div class="wind-speed-and-pressure-div">
                        <h1 v-show="windSpeed!==''">wind speed: {{windSpeed}} <span class="wind-span">m/s</span> </h1>
                        <h1 v-show="pressure!==''">pressure: {{pressure}} <span class="pressure-span">mb</span></h1>

                    </div>
                </div>
            </div>


        </div>



    </div>
</template>

<script>
    import CityNameComp from './CityName.vue'

    export default {
        name:'WeatherDetailsComp',
        components:{
            CityNameComp
        },
        data(){
            return{
                currentCityName:'',
                temp:'',
                pressure:'',
                humidity:'',
                windSpeed:'',
                weatherIcon:'',
                weatherDescription:'',
                //THIS WILL HOLD THE INITIAL BACKGROUND IMAGE WHEN THE PAGE LOADS
                // bgImage:'data:image/jpeg;base64,'
                bgImage:'https://images.pexels.com/photos/10288933/pexels-photo-10288933.jpeg?auto=compress&cs=tinysrgb&w=600&lazy=load'

            }
        },
        methods:{
            fetchCityWeatherDetails(cityName){

                //CHANGING THE BACKGROUND IMAGE
                this.bgImage = `https://source.unsplash.com/1600x900/?${cityName}`

                
                let tempUnit = 'metric'
                let apiKey = 'fdf8813db0cb8371ab10c44a49b00409'
                
                let apiUrl = `https://api.openweathermap.org/data/2.5/weather?q=${cityName}&units=${tempUnit}&APPID=${apiKey}`

                fetch(apiUrl)
                .then((res)=>res.json())
                .then((data)=>{
                    
                    this.currentCityName= cityName
                    this.temp=data.main.temp
                    this.humidity=data.main.humidity
                    this.pressure = data.main.pressure
                    this.weatherDescription = data.weather[0].description

                    let icon = data.weather[0].icon
                    this.weatherIcon = `https://openweathermap.org/img/wn/${icon}.png`

                    this.windSpeed=data.wind.speed

                    
                })
            }
        }

    }
</script>

<style scoped>

    .bg-image-container{
        position: absolute;
        top: 0;
        left: 0;
        height: 100vh;
        width: 100vw;
        z-index: -1;
        
        
    }

    .bg-image-container::after{
        content: '';
        position: absolute;
        top: 0;
        left: 0;
        height: 100%;
        width: 100%;
        background-color: black;
        opacity: .5;
    }

    .bg-image-container img{
        height: 100%;
        width: 100%;
        object-fit: cover;
    }

    /* THE OUTERMOST DIV */
    .root-div{
        display: flex;
        justify-content: center;
        align-content: center;
        display: grid;
        place-items: center;
        height: 100vh;

        border: 8px solid;
    }
    
    .content-container{

        background-color: rgb(24, 22, 22);
        padding: 4rem 1rem;
    }
    .weather-details-container{
        /* border: 1px solid white; */
        padding: 2rem 4rem;
        margin-top: 5rem;
    }


    .title-div h2{
        text-transform: capitalize;
        font-size: 2.4rem;
        padding: .5rem 0;
        color: white;

        font-weight: 900;
    }

    .temp-div h1{
        color: white;
        font-size: 1.8rem;
        padding: .4rem 0;
        font-weight: 600;
    }


    .description-div {
        display: flex;
    }
    .description-div img{
        width: 2rem;
    }
    .description-div h1{
        color: white;
        font-size: 1.5rem;
        text-transform: uppercase;
        font-weight: 600;
        padding: .7rem 0;
        padding-left: .5rem;
    }

    .humidity-div h1{
        color: white;
        padding: .7rem 0;
        font-size: 1.5rem;
        text-transform: capitalize;

    }


    
    .wind-speed-and-pressure-div{
        display: flex;
    }
    .wind-speed-and-pressure-div h1{
        margin-right: 4rem;
        color: white;
        text-transform: capitalize;
        font-size: 1.5rem;
        padding: .7rem 0;
    }
    .wind-speed-and-pressure-div h1 span{
        text-transform: lowercase;
    }



</style>